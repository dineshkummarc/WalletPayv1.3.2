...
const ReceivedCryptoToken = ({ navigation, route }) => {
  const { code = '', typeID = '', type } = route?.params?.currency || {};
  const token =
  ...
  const getAddress = async () => {
    let URL;
    if (type == 'crypto_token') {
      URL = `${config.BASE_URL_VERSION}/token/send/tatumio/user-address/${code}/${typeID}`;
    }
    ...
    if (result) {
      ...
      setRefreshing(false);
      if (status?.code === 200) {
        setAddress(records?.cryptoAddress)
        setError('')
      }
      else {
      ...
      <View style={styles.container}>
        {error && !loading ? (
          ...
          <>
            ...
            <View>
              <Text style={[styles.info, styles.contentWidth]}>
                {trans('Only receive token balance of')}
                <Text style={styles.textBold}> {code} </Text>
                <Text>{trans('to this address, receiving any other Token will result in permanent loss.')}</Text>
              </Text>
            </View>
...
export default ReceivedCryptoToken;
